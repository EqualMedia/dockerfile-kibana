...
ENV KIBANA_TARBALL_URL https://download.elasticsearch.org/kibana/kibana/kibana-3.1.2.tar.gz
RUN wget -q ${KIBANA_TARBALL_URL} && tar -xzf $(basename ${KIBANA_TARBALL_URL}) &&\
  mv $(echo $(basename https://download.elasticsearch.org/kibana/kibana/kibana-3.1.2.tar.gz) | sed "s/\.tar\.gz//")\
